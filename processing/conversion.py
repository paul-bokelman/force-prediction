    ...
    for subject in subjects:
        ...
        if os.path.isdir(subject_dir):
            ...
            if os.path.exists(raw_dir):
                Log.debug(f"Skipping {subject} as raw folder already exists")
                continue
...
def _process(subject: str):
    ...
    raw_dir = constants.raw_data_dir(subject)
    processed_dir = constants.processed_data_dir(subject)
    ...
    for file in os.listdir(raw_dir):
        ...
        mvc_file_path = os.path.join(processed_dir, f"MVC.{mvc_level}.{trial_number}.npy")
        force_file_path = os.path.join(processed_dir, f"FORCE.{mvc_level}.{trial_number}.npy")
        ...
        if os.path.exists(mvc_file_path) and os.path.exists(force_file_path):
            Log.debug(f"\tSkipping {file} as it has already been processed")
            continue

        if mvc_level not in constants.mvc_levels:
            Log.debug(f"\tSkipping {file} as the MVC level {mvc_level} is not in the list of valid MVC levels")
            continue
        ...
        if len(mvc_data) == 0 and mvc_level != 100:
            Log.debug(f"\tSkipping {file} as no MVC data was found")
            continue
        
        force_data = _convert_mv_to_kgf(raw_data["ref_signal"][0])
        np.save(force_file_path, force_data)
        ...
        if mvc_level != 100:
            ...
            np.save(mvc_file_path, activation_time.T)

def process_subjects(subjects: list[str] = fetch_subjects(), overwrite: bool = False):
    ...
    for subject in subjects:
        ...
        if not os.path.exists(constants.raw_data_dir(subject)):
            Log.warn(f"\tSkipping {subject} as raw folder does not exist")
            continue
        
        # overwrite flag is set -> delete the processed folder if it exists (for reprocessing)
        if overwrite:
            Log.warn("\toverwrite flag is set to True. All processed data will be overwritten.")

            # delete the processed folder if it exists
            if os.path.exists(constants.processed_data_dir(subject)):
                shutil.rmtree(constants.processed_data_dir(subject))
        
        # create the 'processed' folder if it doesn't exist
        if not os.path.exists(constants.processed_data_dir(subject)):
            os.makedirs(constants.processed_data_dir(subject), exist_ok=True)

        _process(subject) # process the subject's data and export to their processed folder
...
def _load_subject_data(subject: str):
    ...
    for filename in os.listdir(constants.processed_data_dir(subject)):
        # file name format: MVC.{mvc_level}.{trial_number}.npy
        if "MVC" or "FORCE" in filename:
            # extract mvc level and trial number from the file name
            mvc_level, trial_number = map(int, filename.replace("MVC.", "").replace("FORCE.", "").replace(".npy", "").split("."))
            
            # ensure mvc level is valid
            if mvc_level not in constants.mvc_levels:
                Log.debug(f"Skipping {filename} as the MVC level {mvc_level} is not in the list of valid MVC levels")
                continue
            ...
            try:
                ...
                trial_data = np.load(os.path.join(constants.processed_data_dir(subject), filename))

                # mvc data must be sanitized before being stored
                if data_key == "mvc":
                    trial_data = _sanitize_mvc_data(trial_data)
...
def save_dataframe(subjects: list[str] = fetch_subjects(), overwrite = False):
    ...
    columns = ["subject", "mvc_level", "trial_number", "neuron_data", "force_data"]

    # load all the data for each subject
    for subject in subjects:
        subject_data = _load_subject_data(subject)
